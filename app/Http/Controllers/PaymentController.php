...
class PaymentController extends Controller
{
    ...
    {
        $api_key = config('settings.payuKeyId');
        $api_secret = config('settings.payuKeySecret');

        $api = new Api($api_key, $api_secret);

        try {
            $response = Curl::to('https://sandbox.api.payulatam.com/payments-api/4.0/service.cgi')
